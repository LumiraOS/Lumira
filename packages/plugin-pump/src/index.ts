...
const provider: Provider = {
  ...
  async health(ctx: LumiraContext) {
  ...
  rewards: {
    async status(ctx: LumiraContext, input: RewardsStatusInput) {
      ...
      try {
        ...
        const items: Array<{ label: string; amount?: string; meta?: Record<string, unknown> }> = reclaimable.map((account, i) => {
          const parsed = account.account.data.parsed?.info;
          return {
    ...
    async claim(ctx: LumiraContext, input: RewardsClaimInput) {
      ...
      try {
        ...
        const closeInstructions = reclaimable.map((account) =>
          createCloseAccountInstruction(
